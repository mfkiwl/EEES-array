###BEGIN: Fully implemented reduction using algorithm 2
###TOOL: ${S_AS}
###ARGS: ${FILE} -arch-param dwidth:32,stage:5,bypass:false,pe:32,predicate:2 -o reduce2_full
###TOOL: ${S_SIM}
###ARGS:  -imem 0:uni:reduce2_full -arch-param dwidth:32,stage:5,bypass:false,pe:32,predicate:2 -dump-dmem -dump-dmem-prefix reduce2_full -dmem 0:pe:reduce2_full.pe.dmem_init
###MDUMP: reduce2_full.baseline.scalar.dump
###END:

#TODO: update test-case info

add r1, r0, 0	#	 #		# load address of vector
add r2, r0, 0	#	 #		# store address of vector
add r3, r0, 16	#16	 #16	# number of vectors
add r4, r0, 3	#3	 #2		# up to wich round should we reduce?  --> max log_2(num_vect)-1
jal $REDUCE2				# destroys r5 - r10 (except r9 for the return address)
nop	


add r1, r0, 16	#16	 #8		# length of segment
add r2, r0, 2	#2	 #4		# number of segments (aka nPE/segment_length)
add r3, r0, 0	#	 #		# start address of vector (on PE) 
add r4, r0, 1	#1	 #2		# number of vectors
add r8, r0, 0	#	 #		# start address for storing on CP
jal $GATHER2        		# destroys r5,r6,r7
nop
...
# r9 = reserved for return address
