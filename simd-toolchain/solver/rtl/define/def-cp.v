...
`define DEF_CP_I_MEM_ADDR_WIDTH      {{ int(max(19, math.log(cfg.cp.mem['imem_size'], 2)+2)) }}
// bit width of data-memory address: N>16
`define DEF_CP_D_MEM_ADDR_WIDTH      {{ int(max(18, math.log(cfg.cp.mem['dmem_size'], 2)+2)) }}
// address width of data-memory
`define DEF_CP_RAM_ADDR_BITS         {{ int(math.log(cfg.cp.mem['dmem_size'], 2)) }}
// data width
`define DEF_CP_DATA_WIDTH            {{cfg.cp.datapath['data_width']}}
// instruction width
`define DEF_CP_INS_WIDTH             24

`define DEF_CP_IMEM_SIZE             {{cfg.cp.mem['imem_size']}}

`define DEF_CP_DMEM_SIZE             {{cfg.cp.mem['dmem_size']}}
...
`define DEF_CP_RF_INDEX_WIDTH     {{cfg.cp.rf_id_width()}}
`define DEF_CP_RF_DEPTH           {{cfg.cp.rf['size']}}
