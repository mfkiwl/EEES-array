...
`define DEF_PE_NUM                {{cfg.pe.size}}

// bit width of instruction-memory address, 2^(N-2) entries, N>16!!!
`define DEF_PE_I_MEM_ADDR_WIDTH   {{ int(max(19,math.log(cfg.pe.mem['imem_size'], 2)+2)) }}
// bit width of data-memory address: N>16
`define DEF_PE_D_MEM_ADDR_WIDTH   {{ int(max(18, math.log(cfg.pe.mem['dmem_size'], 2)+2)) }}
// address width of data-memory
`define DEF_PE_RAM_ADDR_BITS      {{ int(math.log(cfg.pe.mem['dmem_size'], 2)) }}
// data width
`define DEF_PE_DATA_WIDTH         {{cfg.pe.datapath['data_width']}}
// instruction width
`define DEF_PE_INS_WIDTH          24

`define DEF_PE_IMEM_SIZE          {{cfg.pe.mem['imem_size']}}

`define DEF_PE_DMEM_SIZE          {{cfg.pe.mem['dmem_size']}}
...
`define DEF_RF_INDEX_WIDTH     {{cfg.pe.rf_id_width()}}
`define DEF_RF_DEPTH           {{cfg.pe.rf['size']}}
