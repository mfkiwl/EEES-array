  ...
  for (i = 0; i < number; i++) {
    if (bit_count == 0) {
      ...
      if (wwindow == 0xFF)
        ...
        case 0xFF:
          ERR("%ld:\tERROR:\tRan out of bit stream\n", FTELL());
          aborted_stream();
          break;
        case 0x00: stuffers++; break;
        default:
          if (RST_MK(0xFF00 | aux))
            ERR("%ld:\tERROR:\tSpontaneously found restart!\n",
                FTELL());
          ERR("%ld:\tERROR:\tLost sync in bit stream\n", FTELL());
          aborted_stream();
          break;
        }
...
void clear_bits(void) { bit_count = 0; }
...
int load_quant_tables() {
  ...
  for (i = 0; i < n; i++) {
    aux = FGETC();
    if (first_quad(aux) > 0) {
      ERR("\tERROR:\tBad QTable precision!\n");
      return -1;
    }
    id = second_quad(aux);
    ...
    if (QTable[id] == NULL) {
      ERR("\tERROR:\tCould not allocate table storage!\n");
      exit(1);
    }
    QTvalid[id] = 1;
    for (x = 0; x < 64; x++) { QTable[id]->linear[x] = FGETC(); }
  }
  return 0;
}
...
int init_MCU(void) {
  ...
  for (i = 0; i < n_comp; i++) {
    ...
    for (j = 0; j < n; j++) {
      ...
      if (MCU_buff[k] == NULL) {
        ERR("\tERROR:\tCould not allocate MCU buffers!\n");
        exit(1);
      }
      k++;
      if (k == 10) {
        ERR("\tERROR:\tMax subsampling exceeded!\n");
        return -1;
      }
    }
  }
...
int process_MCU() {
  ...
  if (MCU_column == mx_size) {
    ...
    MSG("%ld:\tINFO:\tProcessing stripe %d/%d\n", FTELL(), MCU_row+1, my_size);
  }
